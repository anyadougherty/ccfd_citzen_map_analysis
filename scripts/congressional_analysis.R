...
sub_plan <- read.csv('../Congressional Redistricting/v2/block assignments 2/bipartisan_minimal_variance_plan_by_brandon_bechtel_2021-11-22_v2.csv')
...
plans = plans %>% mutate(Compactness = comp_polsby(pl(), rd_map_prec),
                               `Population deviation` = plan_parity(rd_map_prec),
                               `Democratic vote` = group_frac(rd_map_prec, ndv, (ndv + nrv)),
                                mean_media = part_mean_median(pl(), rd_map_prec, dvote = ndv, rvote = nrv))
...
mean_media = part_mean_median(plans = rd_map$DISTRICTNO, shp = rd_map, dvote = dem, rvote = rep)

# plot congressional plan
sub_map <- redist.plot.map(rd_map, adj=adj, plan = DISTRICTNO, boundaries = is.null(fill), 
                           title = "Brandon Bechtel's Submission")

png("/outputs_bipartisan_minimal_variance_plan_by_brandon_bechtel_2021-11-22_v2/sub_map.png", width = 800, height = 600)
print(sub_map)
dev.off()
...
png("/outputs_bipartisan_minimal_variance_plan_by_brandon_bechtel_2021-11-22_v2/sub_map_comp_his.png", width = 800, height = 600)
print(comp_his)
dev.off()
...
png("/outputs_bipartisan_minimal_variance_plan_by_brandon_bechtel_2021-11-22_v2/compactness_compared.png", width = 800, height = 600)
print(comp_sub)
dev.off()
...
mean_his <- ggplot(plans, aes(x=mean_media)) + 
  geom_histogram() +
  geom_vline(xintercept = mean_media, 
             color="blue", linetype="dashed", size=1) +
  ggtitle("Histogram of Simulated Plan Mean-Median Scores")

png("/outputs_bipartisan_minimal_variance_plan_by_brandon_bechtel_2021-11-22_v2/sub_map2_mean_median.png", width = 800, height = 600)
print(mean_his)
dev.off()
...
png("/outputs_bipartisan_minimal_variance_plan_by_brandon_bechtel_2021-11-22_v2/mean_median_compared.png", width = 800, height = 600)
